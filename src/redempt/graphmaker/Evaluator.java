...
public class Evaluator {
	...
	public Evaluator(String expression) {
		this.expression = expression.replace(" ", "").trim();
	...
	public double evaluate(double input) {
		...
		String expression = this.expression.replace(" ", "").replaceAll("(\\d|\\))x", "$1*" + format.format(input)).replace("x", format.format(input));
		...
		try {
			...
			while (expression.contains("+") || expression.contains("-")) {
				int first = expression.contains("+") ? expression.indexOf("+") : expression.indexOf("-");
				if (expression.substring(1).contains("-") && expression.charAt(first) == '-') {
